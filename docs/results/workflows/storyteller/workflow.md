...
1. **Read pipeline state:** `{project-root}/.bmad/creative-studio/_state/creative-pipeline-state.yaml`
2. **Find diverse-content-gen tracking file:** Extract `tracking_file` from stage 01
3. **Load tracking YAML:** Read the content-gen-{timestamp}.yaml file
4. **Extract handoff metadata:** Get all required fields (emotional_core, pov, setting, tone, etc.)
5. **Load content file:** Read the selected idea from the content markdown
...
**File:** `outputs/scenes-{timestamp}.md`
